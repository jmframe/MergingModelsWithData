...
dim = 2; OF = 10000000; numOF = 2; DEN = 40;
maxn = 1; minn = 0;
ObjVals = ones(2*DEN, numOF); TrialVals = ObjVals;
pfront = zeros(2,2);
CR = 0.5; r1 = 0; r2 = 0; r3 = 0; F = 0.2; K = 0.8;
...
for ii = 1:10
    ...
    for i = 1:DEN  %Objective Values
        ObjVals(i,1) = OF1(x(i,:)); ObjVals(i,2) = OF2(x(i,:));
    end

    for i = DEN + 1:2*DEN  % Trial matrix Objective Values
        ObjVals(i,1) = OF1(U(i-DEN,:)); ObjVals(i,2) = OF2(U(i-DEN,:));
        x(i,:) = U(i-DEN,:);
    end
    ...
    scatter(ObjVals(1:DEN,1),ObjVals(1:DEN,2), 'b'); hold on...
        ;scatter(ObjVals(DEN:DEN*2,1), ObjVals(DEN:DEN*2,2),5, 'r'),
